...
public class TankShooterGame implements Game {
	...
	public void addTank(NetworkPlayer player) {
		...
		for (int i = 0; i < 8; i++) {
			tankSprites[i] = spriteSheet.getSprite(i, player.getNum());
		}

		TankEntity tank = new TankEntity(player.getName(), player.getNum(), this, tankSprites, 10, 50 + 50 * player.getNum());
		entities.add(tank);
	}
	...
	public TankEntity getTank(NetworkPlayer player) {
		for(Entity entity : entities) {
			if(entity instanceof TankEntity) {
				TankEntity tank = (TankEntity) entity;
				if(player.getNum()==tank.getNum()) {
					return tank;
				}
			}
		}
